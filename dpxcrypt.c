...
void
AES_cbc_set_key (AES_CONTEXT *ctx, unsigned int keylen, const unsigned char *key)
{
                 ...
                 unsigned char      **cipher, size_t *cipher_len)
{
  ...
  unsigned char *outptr, block[BLOCKSIZE], iv[BLOCKSIZE];
  int i, len;
  int padbytes = BLOCKSIZE - (plain_len % BLOCKSIZE);

  for (i = 0; i < BLOCKSIZE; i++)
    iv[i] = rand() % 0xff;

  *cipher_len = plain_len + 16 + padbytes; /* IV and padding bytes */
  *cipher     = NEW(*cipher_len, unsigned char);
  
  inptr = plain; outptr = *cipher;
  memcpy(outptr, iv, BLOCKSIZE);
  outptr += BLOCKSIZE;
  for ( len = plain_len; len >= BLOCKSIZE; len -= BLOCKSIZE) {
    for (i = 0; i < BLOCKSIZE; i++)
      block[i] = inptr[i] ^ iv[i];
    rijndaelEncrypt(ctx->rk, ctx->nrounds, block, outptr);
    memcpy(iv, outptr, BLOCKSIZE);
    inptr  += BLOCKSIZE;
    outptr += BLOCKSIZE;
  }
  for (i = 0; i < BLOCKSIZE; i++)
    block[i] = ((i < len) ? inptr[i] : padbytes) ^ iv[i];
  rijndaelEncrypt(ctx->rk, ctx->nrounds, block, outptr);
  inptr  += BLOCKSIZE;
  outptr += BLOCKSIZE;
}
