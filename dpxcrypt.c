...
/* 
 * The following codes for the SHA256 has function were taken from
...
SHA256_final(unsigned char *outbuf, SHA256_CONTEXT *hd)
{
  ...
  p = outbuf;
#define X(a) do { *(u32*)p = be_bswap32(hd->a); p += 4; } while(0)
  X(h0);
  X(h1);
  X(h2);
  X(h3);
                 ...
                 unsigned char      **cipher, size_t *cipher_len)
{
  ...
  *cipher     = NEW(*cipher_len, unsigned char);
  ...
  for (i = 0; i < BLOCKSIZE; i++)
    block[i] = ((i < len) ? inptr[i] : padbytes) ^ ctx->iv[i];
  rijndaelEncrypt(ctx->rk, ctx->nrounds, block, outptr);
