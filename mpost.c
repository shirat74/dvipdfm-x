...
static int mp_parse_body_old (const char **start, const char *end, double x_user, double y_user);
...
do_mpost_bind_def (const char *ps_code, double x_user, double y_user)
{
  ...
  error = mp_parse_body_old(&start, end, x_user, y_user);
 ...
 */
static int
mp_parse_body_old (const char **start, const char *end, double x_user, double y_user)
{
  char    *token;
  pdf_obj *obj;
  int      error = 0;
...
static int
mps_stack_depth_old (void)
{
  return top_stack;
}

static int
mps_exec_inline_old (const char **p, const char *endptr, double x_user, double y_user)
{
  int  error;
  ...
  error = mp_parse_body_old(p, endptr, x_user, y_user);
...
mps_do_page (FILE *image_file)
{
  ...
  error = mp_parse_body_old(&start, end, 0.0, 0.0);
